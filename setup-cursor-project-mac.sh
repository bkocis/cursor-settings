#!/usr/bin/env bash
#
# Setup a Python venv and Cursor/VSCode config for the current project (macOS).
...
# - Replaces <ENV_PATH> in copied files with the venv path (configure_environment_on_MAC)
#
# Usage:
#   cd /path/to/your/project
#   ./setup-cursor-project-mac.sh [TEMPLATE_DIR]
#
# On your local Mac: set CURSOR_TEMPLATE_DIR below or export it.
# To run anywhere: pass TEMPLATE_DIR as first argument or set CURSOR_TEMPLATE_DIR.
#

set -e

# --- Configure on your local Mac: path to cursor-setup-template ---
CURSOR_TEMPLATE_DIR="${CURSOR_TEMPLATE_DIR:-$HOME/Documents/Projects/cursor-setup-template}"
...
# Replace <ENV_PATH> in rule files and settings (same as configure_environment_on_MAC)
# macOS BSD sed: -i '' for in-place, no backup
